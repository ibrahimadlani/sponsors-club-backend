...
def test_user_manager_account_type_defaults(user_model):
    ...
    superuser = user_model.objects.create_superuser(email='admin@example.com', password='pass')
...
def test_me_update_display_name_only(api_client, agent_user):
    serializer = MeUpdateSerializer(agent_user, data={'display_name': 'Solo Update'}, partial=True, context={'request': None})
...
@pytest.mark.django_db
def test_roles_endpoint_includes_collaborations(api_client, owner_user, organisations_setup):
    ...
    assert response.data['collaborations'][0]['organisation_id'] == str(organisations_setup['organisation'].id)
...
def test_roles_builder_with_agent_and_owner(agent_user, organisations_setup):
    ...
    assert data['agent_profile']['display_name'] == agent_user.agent_profile.display_name
...
def test_user_str_representation(agent_user, user_model):
    ...
    plain_user = user_model.objects.create_user(email='plain-no-name@example.com', password='pass')
...
def test_login_and_me_flow(api_client, owner_user):
    ...
    response = api_client.post(login_url, {'email': owner_user.email, 'password': 'pass1234'}, format='json')
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_me_entitlements_collaborator(api_client, owner_user, organisation_subscription):
    url = reverse('users:me_entitlements')
    api_client.force_authenticate(user=owner_user)
    response = api_client.get(url)
    assert response.status_code == 200
