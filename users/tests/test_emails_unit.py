...
def test_send_email_verification_uses_template_and_context(
    monkeypatch, settings, user_model
):
    settings.EMAIL_VERIFICATION_URL_TEMPLATE = (
        "https://example.com/verify/{uid}/{token}"
    )

    user = user_model.objects.create_user(
        email="template@example.com", password="pass1234"
    )
    ...
    captured_messages = []
    monkeypatch.setattr(
        user_emails, "send_email", lambda message: captured_messages.append(message)
    )
    ...
    assert rendered_contexts[
        "verification_url"
    ] == "https://example.com/verify/{uid}/{token}".format(
        uid=str(user.id),
        token="issued-token",
    )
...
@pytest.mark.django_db
def test_send_email_verification_logs_delivery_error(
    monkeypatch, settings, user_model, caplog
):
    settings.EMAIL_VERIFICATION_URL_TEMPLATE = None
    user = user_model.objects.create_user(
        email="failure@example.com", password="pass1234"
    )

    monkeypatch.setattr(
        user_emails.EmailVerificationToken, "issue_for_user", lambda _: "token"
    )
    monkeypatch.setattr(
        user_emails, "render_to_string", lambda *_args, **_kwargs: "body"
    )
    ...
    assert any(
        "Failed to send verification email" in record.getMessage()
