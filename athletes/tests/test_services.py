...
def create_sport(name: str, *, emoji: str = "🏅") -> Sport:
    ...
    SportDiscipline.objects.create(
        sport=sport, name=f"{name} Discipline", slug=f"{name}-d"
    )
    return sport
...
def test_base_athlete_queryset_returns_all(agent_user):
    ...
    athlete_one = create_athlete(
        agent=agent_user.agent_profile, sport=sport, name="Alice Player"
    )
    athlete_two = create_athlete(
        agent=agent_user.agent_profile, sport=sport, name="Bob Player"
    )
...
def test_my_athletes_queryset_filters_by_agent(agent_user, user_model):
    ...
    mine = create_athlete(
        agent=agent_user.agent_profile, sport=sport, name="Home Athlete"
    )
...
def test_athlete_cards_payload_supports_filters(agent_user):
    ...
    create_athlete(
        agent=agent_user.agent_profile, sport=primary_sport, name="Clara Sprinter"
    )
    create_athlete(
        agent=agent_user.agent_profile, sport=primary_sport, name="Adele Rider"
    )
    target = create_athlete(
        agent=agent_user.agent_profile, sport=secondary_sport, name="Bella Diver"
    )
    ...
    payload_by_sport = athlete_cards_payload(
        AthleteCardFilters(sport_id=str(primary_sport.id))
    )
...
def test_sport_disciplines_payload_returns_nested_data():
    ...
    assert [d["name"] for d in payload["disciplines"]] == [
        "Bike",
        "Swim",
...
def test_athlete_photos_payload_returns_gallery(agent_user):
    ...
    athlete = create_athlete(
        agent=agent_user.agent_profile, sport=sport, name="Gallery Star"
    )

    image_file = SimpleUploadedFile(
        "photo.png", b"fake image", content_type="image/png"
    )
    photo = AthletePhoto.objects.create(
        athlete=athlete, image=image_file, caption="Lead Wall"
    )
