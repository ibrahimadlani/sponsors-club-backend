...
## Release automation
Version tags follow [Semantic Versioning](https://semver.org/). A dedicated GitHub Actions workflow (`Release`) automates tag creation and GitHub releases:

1. Open the repository’s **Actions** tab and select **Release**.
2. Click **Run workflow**, choose the increment (`patch`, `minor`, or `major`), and optionally provide a pre-release suffix such as `rc1`.
3. The workflow calculates the next SemVer tag, pushes it back to the repository, and publishes a release with autogenerated notes.

The workflow falls back to `0.0.0` when no prior tag exists, guaranteeing the first run produces `0.0.1`. Subsequent runs always build on the latest tag in `main`.
